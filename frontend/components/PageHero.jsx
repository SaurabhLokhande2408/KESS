  ...
  return (
    <section className="relative overflow-hidden bg-ivory text-charcoal border-b border-gold/30">
      ...
      <div className="relative z-10 mx-auto max-w-7xl grid lg:grid-cols-2 items-stretch">
        {/* LEFT: copy */}
        <div className="px-6 sm:px-12 lg:pl-16 lg:pr-10 py-14 sm:py-20 lg:py-24 flex flex-col justify-center">
          {eyebrow && (
            <p className="font-display italic text-charcoal text-sm sm:text-base font-medium tracking-[0.08em] mb-5">
              {eyebrow}
            </p>
          )}

          <h1 className="font-display uppercase font-normal leading-[0.95] tracking-[-0.025em] text-charcoal text-[2rem] sm:text-5xl lg:text-[3.4rem]">
            {lines.map((line, i) => (
          ...
          {description && (
            <p className="mt-7 max-w-md text-charcoal-light text-base sm:text-lg leading-8">
              {description}
            </p>
          )}
            ...
            <div className="mt-10 flex flex-wrap items-center gap-6">
              {primaryCta && (
                <Link
                  ...
                  className="bg-charcoal text-ivory px-7 py-3.5 uppercase tracking-[0.15em] text-sm font-semibold hover:bg-gold hover:text-charcoal transition-colors"
                >
                  {primaryCta.label}
                </Link>
              )}
              {secondaryCta && (
                <Link
                  ...
                  className="uppercase tracking-[0.15em] text-sm font-medium text-charcoal-light underline underline-offset-4 decoration-gold hover:text-gold transition-colors"
                >
                  {secondaryCta.label}
                </Link>
              )}
